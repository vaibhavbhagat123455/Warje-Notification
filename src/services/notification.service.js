...
export const sendNotificationForCase = async (caseId, notificationDay, payload = null) => {
    ...
    for (const u of users) {
        if (!u.users?.fcm_token) {
            console.warn(`⚠️ User ${u.user_id} has no FCM Token. Skipping.`);
            continue;
        }

        try {
            await firebase.messaging().send({
                token: u.users.fcm_token,
                notification: {
                    title: messageTitle,
                    body: messageBody
                },
            ...
            console.log(`Sent notification for Case ${caseData.case_number} (Day ${notificationDay}) to Token ending in ...${u.users.fcm_token.slice(-4)}`);
        } catch (e) {
            console.error(`Failed to send FCM to user ${u.user_id}:`, e.message);
        }
    }
};
